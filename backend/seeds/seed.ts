...
async function seed() {
  ...
  if (emp && agent && cat) {
    await pool.query(`INSERT INTO tickets (title, description, category_id, priority, status, created_by, assigned_agent_id, assigned_team_id,
      response_deadline, resolution_deadline)
      VALUES
      ('Laptop not connecting to VPN', 'My laptop cannot connect to the company VPN since this morning.', $1, 'high', 'in_progress', $2, $3, '11111111-1111-1111-1111-111111111111',
        NOW() + INTERVAL '30 minutes', NOW() + INTERVAL '8 hours'),
      ('Software installation failing', 'Adobe Acrobat installation keeps failing with error code 1603.', $1, 'medium', 'open', $2, NULL, NULL,
        NOW() + INTERVAL '2 hours', NOW() + INTERVAL '24 hours'),
      ('Laptop screen damaged', 'My laptop screen has a crack and is barely visible.', $1, 'critical', 'open', $2, NULL, '11111111-1111-1111-1111-111111111111',
        NOW() + INTERVAL '15 minutes', NOW() + INTERVAL '4 hours')
      ON CONFLICT DO NOTHING`,
